...
function mission(runId, targetUrl) {
...
async function main() {
  ...
  const stableKey = `trialroom-${createHash("sha256")
    .update(`${repository}\0${targetUrl}\0${runId}`)
    .digest("hex")}`;
  ...
  try {
    const created = await (
      await request("/sandboxes", {
        ...
        body: JSON.stringify({
          ...
          prompt: mission(runId, targetUrl),
          model: "sonnet",
    ...
    const state = {
      ...
      status: created.status ?? created.sandbox?.status,
    };
    ...
    console.log(`Screenshot egress verified: ${screenshotBase64.replaceAll(/\s/g, "").length} base64 characters`);
  } finally {
    if (sandboxId) {
      try {
