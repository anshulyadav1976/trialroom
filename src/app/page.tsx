...
function LiveFindings({ study }: { study: EvidenceStudy }) {
  const rows = study.clusters.length ? study.clusters.map((cluster) => {
    ...
    return { id: cluster.id, title: cluster.title, detail: cluster.summary, severity: cluster.severity, count: cluster.affectedCount, finding: members[0] };
  }) : study.findings.map((finding) => ({ id: finding.id, title: finding.observation.summary, detail: finding.observation.actual, severity: finding.severity, count: 1, finding }));
  const [selected, setSelected] = useState(rows[0]?.id ?? "");
  const active = rows.find((row) => row.id === selected) ?? rows[0];
  if (!active) return <div className="view live-evidence-empty"><span className="eyebrow">Live findings</span><strong>No friction was returned</strong><p>The completed tester results contain no findings, so TrialRoom is not inventing any.</p></div>;
  const evidence = active.finding?.evidence[0];
  return <div className="view findings-view"><div className="filterbar"><span>{rows.length} evidence-backed {rows.length === 1 ? "finding" : "findings"}</span><button className="selected">All</button><button>Live result</button></div><div className="findings-layout"><section className="findings-list">{rows.map((row, index) => <button key={row.id} className={active.id === row.id ? "finding-row active" : "finding-row"} onClick={() => setSelected(row.id)}><span className={`finding-index ${row.severity.toLowerCase()}`}>{String(index + 1).padStart(2, "0")}</span><span><small>{row.severity} · {row.count} tester{row.count === 1 ? "" : "s"}</small><strong>{row.title}</strong><p>{row.detail}</p><em>{row.finding ? `${row.finding.route} · ${row.finding.stepId} · ${row.finding.category}` : "Clustered tester evidence"}</em></span><Icon name="arrow"/></button>)}</section><aside className="finding-detail"><div className="detail-top"><span className={`severity ${active.severity.toLowerCase()}`}>{active.severity.slice(0, 1).toUpperCase()}</span><span><small>{active.severity} severity</small><strong>{active.count} / {study.testers.length} testers</strong></span></div><h2>{active.title}</h2><p>{active.detail}</p><div className="detail-shot live-detail-shot">{evidence?.screenshot?.url ? <img src={evidence.screenshot.url} alt={evidence.screenshot.label || `Evidence for ${active.title}`}/> : <span>No screenshot URL returned</span>}</div>{active.finding && <><span className="eyebrow">Browser evidence</span><div className="evidence-note"><b>Page</b><span>{evidence?.pageUrl ?? active.finding.route}</span><b>Action</b><span>{evidence?.attemptedAction ?? "Not recorded"}</span><b>Observed</b><span>{active.finding.observation.actual}</span></div><span className="eyebrow">Reproduce</span><ol>{active.finding.reproduction.map((step) => <li key={step}>{step}</li>)}</ol></>}</aside></div></div>;
}
