...
export default function Home() {
  ...
  return <main className="app-shell">
    ...
    <section className="workspace">
      <header className="topbar"><div><span className="eyebrow">Synthetic product study · {mode === "preview" ? "Preview data" : liveRun?.seeded ? "Seeded evidence" : "Live Sparkles"}</span><h1>{view === "overview" ? "The room at a glance" : view === "testers" ? "Four perspectives, live" : view === "findings" ? "Evidence worth acting on" : "Every journey, in context"}</h1></div><button className="run-button" onClick={mode === "preview" ? runDemo : liveRun?.seeded ? newStudy : runLive} disabled={running}><Icon name="play"/>{mode === "preview" ? previewRunning ? `Running · ${phase}/5` : phase === 6 ? "Run preview again" : "Run preview" : liveRun?.seeded ? "Start new study" : starting ? "Starting four rooms" : liveRunning ? "Study running" : liveRun ? "Run live again" : "Run live study"}<span><Icon name="arrow"/></span></button></header>
